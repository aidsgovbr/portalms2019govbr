 ...
 * @since  1.6
 */
class PlgSearchRemoveShortcode extends JPlugin {
    ...
    public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null) {
        ...
        foreach ($results as $article) {
            ...
            $article->text = preg_replace_callback('/' . $pattern . '/s', 'remove_callback', $article->text);
        }
...
function remove_callback($m) {
    $pattern = su_get_shortcode_regex();
    if (isset($m[5])) {
        // enclosing tag - extra parameter
        return preg_replace_callback('/' . $pattern . '/s', 'remove_callback', $m[5]);
    } else {
        // self-closing tag
        return '';
    }
