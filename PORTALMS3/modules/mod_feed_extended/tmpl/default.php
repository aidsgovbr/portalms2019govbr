...
else
{
	...
	<?php if (!empty($feed))
	{
		$rssdate = $params->get('rssdate', '');
		$rssitemtransition = $params->get('rssitemtransition', '');
	 ...
	 ?>
					...
					<?php if (!empty($uri)) :
						...
						if(!empty($rssdate)){ 
							...
							try {
								...
								$date = new DateTime($feed[$i]->updatedDate->format($rssdate));
							   
							   	if(!$date)
							    	throw new Exception($error);

							    echo '<h4 class="feed-date">'.$date->format($rssdate).'</h4>';

							} catch (Exception $e) {
							    pr(true,"<p>Caught exception: ",  $e->getMessage(), "</p>");
							}
		...
		<?php } ?>
				<?php if(!empty($rsslink)): ?>
					...
					<h4 class="feed-more">
						<a href="<?php echo $rsslink; ?>" target="_blank">
							OUTROS POSTS	
						</a>
					</h4>
