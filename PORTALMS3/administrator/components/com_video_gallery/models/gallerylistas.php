...
class Video_galleryModelGallerylistas extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		$this->setState('filter.tipo_video', $app->getUserStateFromRequest($this->context.'.filter.tipo_video', 'filter_tipo_video', '', 'string'));


        // Load the parameters.
        $params = JComponentHelper::getParams('com_video_gallery');
    ...
    protected function getListQuery() {
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.titulo LIKE '.$search.'  OR  a.tipo_video LIKE '.$search.' )');
            }
        }
