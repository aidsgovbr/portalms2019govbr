 ...
 * @since       1.0
 */
class SearchViewQuero extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		if ($error == null)
		{
			...
			for ($j= 0 ; $j < count($results) ; $j ++){
				...
				// }
					...
					$result[$i]->text    = JHtml::_('content.prepare', $result[$i]->text, '', 'com_search.search');

					$result[$i]->created = $created;
					$result[$i]->count   = $i + 1;
				}
			}
		}
		...
		// dando nomes aos arrays
		$lista = array();
		foreach ($results as $i => $result)
		{
			$lista[$areas['active'][$i]] = $result;
		}
		$results = $lista;
