...
<?php if (!isset($this->item[0])) { ?>
<strong>Nenhum resultado encontrado.</strong>
<p><a href="index.php/busca-por-medico-1"><strong>Voltar</strong></a></p>
<?php } else{ ?>
	...
	<table class="category table table-striped table-bordered table-hover">
		<thead>
			...
			<th id="categorylist_header_indicacao" style="text-align: center;">
...
<form name="enviaresultado" action="resultado" method="get" accept-charset="utf-8">
	<tbody>
		<?php foreach ($this->item as $key => $value) { 
			...
			if ($value['TOTGERAL'] == NULL || $value['TOTGERAL'] == "0") {
				$percentnormal = 0;
				$percentcesaria = 0;
			} else{
				$totnormal = $value['TOTNORMAL'];
				$totgeral = $value['TOTGERAL'];
				$totcesaria = $value['TOTCESARIA'];
...
<div class="pagination pagination-centered">
  ...
  <ul>
    ...
    if ($this->item[0]['PAGINA'] > 1) {
        ...
        echo " <li><a href='{$_SERVER['PHP_SELF']}?currentpage=1&cod_estados=$estado&cod_cidades=$municipio&crm=$crm'><<</a></li> ";
        // get previous page num
        $prevpage = $this->item[0]['PAGINA'] - 1;
        // show < link to go back to 1 page
        #echo " <li><a href='{$_SERVER['PHP_SELF']}?currentpage=$prevpage'><</a> ";
        echo " <li><a href='{$_SERVER['PHP_SELF']}?currentpage=$prevpage&cod_estados=$estado&cod_cidades=$municipio&crm=$crm'><</a></li> ";
    } // end if
    ...
    if ($this->item[0]['PAGINA'] != $this->item[0]['TOTALPAG']) {
        ...
        echo " <li><a href='{$_SERVER['PHP_SELF']}?currentpage=$this->item[0]['TOTALPAG']&cod_estados=$estado&cod_cidades=$municipio&crm=$crm'>>></a></li> ";
     } // end if
