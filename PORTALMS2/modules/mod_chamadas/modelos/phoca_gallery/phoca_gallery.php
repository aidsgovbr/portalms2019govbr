...
class ModeloPhoca_gallery
{
	public function getListaModelo($params)
	{
		...
		$query->clear();
		$query->select('cont.id, cont.catid, cont.alias, cont.date AS created_date,
		 cont.date AS modify_date, cont.date AS publish_date, cont.metadesc AS image_caption, cat.userfolder AS folder');
		...
		for ($i=0; $i < $lista_counter; $i++) {
			...
			elseif($params->get('exibir_imagem')) {
				...
				// implementado por Tiago Garcia 22/06/2017
				if ($params->get('sizeImage') != '' && $params->get('sizeImage') != 'Original') {
					$prefix = 'thumbs/phoca_thumb_'.$params->get('sizeImage').'_';
				}
