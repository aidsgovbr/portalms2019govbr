...
if ($images) {
	...
	foreach($imagesArray as $valueImages){
		...
		$imgLinkOrig = PhocaGalleryFile::getFileOriginal($valueImages->filename, 1);
		...
		$linkCategorias = PhocaGalleryRoute::getCategoriesRoute();
		...
		if ($tmpl['detail_window'] == 2 ) {
			...
			if(isset($valueImages->extid) && $valueImages->extid != '') {
				...
				$tmpl['jakdatajs'][$i] .= "small: {url: '".htmlentities(PhocaGalleryText::strTrimAll(addslashes($thumbLinkM->rel)))."'},"
				."big: {url: '".htmlentities(PhocaGalleryText::strTrimAll(addslashes($imgLink)))."'} }";
			}
		}
		...
		switch($module_type) {
			...
			case 1:
				...
				$output[$i] .= '<div class="span6 span-inline">';
				...
				if (isset($valueImages->extid) && $valueImages->extid != '') {
					...
					$output[$i] .= '<img src="'.$valueImages->linkthumbnailpath.'" alt="'.$valueImages->title.'" />';
				}
				$output[$i] .= '</a>';
...
require(JModuleHelper::getLayoutPath('mod_phocagallery_egov'));
?>