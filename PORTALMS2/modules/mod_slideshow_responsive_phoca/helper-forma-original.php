...
class modSlideshowResponsivePhoca {
	...
	public static function getList(&$params){
		...
		JHTML::stylesheet( 'media/mod_phocagallery_egov/css/phocagallery.css' );
		...
		//Additional variable take from parameters for image size override KM 08-05-12
		...
		if ($module_type == 0) {
			$document->addCustomTag( "\n" ."<style type=\"text/css\">\n"
				...
				." #phocagallery-module-ri .pg-cv-box:hover, .pg-cv-box.hover {border:1px solid $border_color_hover ;
		...
		$btn->sbSettings			= $paramsC->get( 'sb_settings', "overlayColor: '#000',overlayOpacity:0.5,resizeDuration:0.35,
		...
		if (isset($button->highslideonclick)) {
			$tmpl['highslideonclick'] = $button->highslideonclick;
		}
		$tmpl['highslideonclick2']	= '';
		if (isset($button->highslideonclick2)) {
			$tmpl['highslideonclick2'] = $button->highslideonclick2;
		}
		...
		cc.accessuserid as accessuserid'
		...
		foreach ($categories as $key => $category) {
			...
			if (isset($categories[$key])){
				$rightDisplay = PhocaGalleryAccess::getUserRight('accessuserid', $categories[$key]->accessuserid,
					$categories[$key]->access, $user->getAuthorisedViewLevels(), $user->get('id', 0), 0);
			}
			if ($rightDisplay == 0) {
		...
		if ($images) {
			...
			$query = 'SELECT cc.id, cc.alias as catalias, a.id, a.catid, a.title, a.alias, a.filename, a.description, a.extm, a.exts,a.extl,
			a.exto, a.extw, a.exth, a.extid,'
			...
			$categoria = $imagesArray[0]->categoria;
			...
			// Different Link - to all category
				...
				// links DESSA GALERIA  e OUTRAS GALERIAS, by Tiago.
				$linkDestaque = PhocaGalleryRoute::getCategoryRoute($valueImages->catid, $valueImages->catalias);
			...
			// Mosaic
				...
				// $output[$i] .= '<div class="span6' . (($i == 2) ? "spanleft0" : "") .'">';
					// $output[$i] .= '<div class="span12 span-inline">';
				...
				//End Slimbox Method
					...
					if (isset($valueImages->extid) && $valueImages->extid != '') {
						$correctImageRes = PhocaGalleryPicasa::correctSizeWithRate($valueImages->extw, $valueImages->exth, $small_image_width, $small_image_height);
						$output[$i] .= '<img src="'.$valueImages->exts.'" alt="'.$valueImages->title.'" />';
					} else {

						$output[$i] .= '<img src="'.JURI::base(true).'/'.$valueImages->linkthumbnailpath.'" alt="'.$valueImages->title.'" />';
					}
					$output[$i] .= '</a>';
...
// montando as imagens.
					...
					$output[$i] .= '<img src="'.$valueImages->extm.'" alt="'.$valueImages->title.'" />';
				} else {
				...
				if ($tmpl['display_name'] == 1) {
					$output[$i] .= '<div class="phocaname" style="text-align:center;color: '.$font_color.' ;font-size:'.$font_size_name.
					'px;">'.PhocaGalleryText::wordDelete($valueImages->title, $char_length_name, '...').'</div>';
				}
	...
	// fim foreach
		...
		$i = 0;
			// there is no image to get it as random image
		$output[$i] = '';
	}
